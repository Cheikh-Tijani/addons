...
var UserMenu = Widget.extend({
    ...
    on_menu_documentation: function () {
        window.open('https://www.groupesepro.com/documentation/user', '_blank');
    },
    on_menu_support: function () {
        window.open('https://www.groupesepro.com/buy', '_blank');
    },
    ...
    on_menu_account: function() {
        this.getParent().clear_uncommitted_changes().then(function() {
            ...
            P.call('get_param', ['database.uuid']).then(function(dbuuid) {
                ...
                framework.redirect('https://accounts.groupesepro.com/oauth2/auth?'+$.param(params));
            }).fail(function(result, ev){
                ev.preventDefault();
                framework.redirect('https://accounts.groupesepro.com/account');
            });
        });
    },
